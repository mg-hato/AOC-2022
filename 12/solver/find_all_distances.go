...
func findAllDistancesFromGoalPosition(terrain [][]m.Field) map[m.Field]int {
	...
	goals := c.Filter(func(field m.Field) bool { return field.HeightCode == 'E' }, c.Flatten(terrain))
	if len(goals) == 0 {
	...
	position_within_bounds := func(position m.Position) bool {
		return c.InRange(0, len(terrain))(position.First) && c.InRange(0, len(terrain[0]))(position.Second)
	}
	...
	queue_up_neighbours := func(field m.Field) {
		queue = append(queue,
			c.Map(position_to_field, c.Filter(func(position m.Position) bool {
	...
	for i < len(queue) {
		...
		reachable_neighbours := c.Filter(
			...
			c.Map(position_to_field, c.Filter(position_within_bounds, m.GetNeighbours(field.Position))),
		)
		...
		closest_neighbour := c.MinimumBy(reachable_neighbours, func(lhs, rhs m.Field) bool {
